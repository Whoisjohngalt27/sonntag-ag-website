    ...
    <div className="bg-white py-24 md:py-32 lg:py-40">
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-20 px-6 lg:px-8 xl:grid-cols-3">
        <div className="mx-auto max-w-2xl lg:mx-0">
          <h2 className="text-4xl font-semibold tracking-tight text-pretty text-black sm:text-5xl">Unser Team</h2>
          <p className="mt-6 text-lg/8 text-gray-600">
            Wir sind ein dynamisches Team von Experten, die leidenschaftlich an der Beratung von Unternehmen arbeiten und 
            unseren Kunden die besten Ergebnisse liefern.
          </p>
        </div>
        <ul
          role="list"
          ...
          {people.map((person) => (
            <li key={person.name}>
              <img
                ...
                src={person.imageUrl}
                className="aspect-3/2 w-full rounded-2xl object-cover outline-1 -outline-offset-1 outline-gray-200"
              />
              <h3 className="mt-6 text-lg/8 font-semibold text-black">{person.name}</h3>
              <p className="text-base/7 text-gray-600">{person.role}</p>
              <p className="mt-4 text-base/7 text-gray-500">{person.bio}</p>
