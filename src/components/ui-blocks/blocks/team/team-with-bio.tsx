/**
 * Component: Team with Bio
 * Source: Tailwind CSS Plus - Team Section
 * Adaptation: Original dark theme version
...
export default function Example() {
  return (
    <div className="bg-gray-900 py-24 md:py-32 lg:py-40">
          ...
          {people.map((person) => (
            <li key={person.name}>
              <img
                alt=""
                src={person.imageUrl}
                className="aspect-3/2 w-full rounded-2xl object-cover outline-1 -outline-offset-1 outline-white/10"
              />
              <h3 className="mt-6 text-lg/8 font-semibold text-white">{person.name}</h3>
              <p className="text-base/7 text-gray-400">{person.role}</p>
              <p className="mt-4 text-base/7 text-gray-400">{person.bio}</p>
